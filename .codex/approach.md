...
**Status**: done
**Last updated**: 2026-07-17T07:57:57+07:00
...
- CWD: repository root
- Git repo: yes; `main` is published to `git@github.com:jedt3d/fpcXUI.git`
- Branch: `main`
...
Failure pre-mortem: likely failures were VS Code archive/signature mismatch, Node 26 incompatibility with pinned test tooling, LSP framing corruption from stdout logs, UTF-16 errors on surrogate pairs, parser spike silently accepting only declarations, and Windows-specific FPC/package behavior. The remote matrix exposed the missing Windows `fpc.cfg` and RawByteString transcoding cases; both are now regression-covered.
...
- Cross-platform CI passes the TypeScript client and Pascal harness on Ubuntu, macOS, and Windows.
...
| 2026-07-17T07:35:00+07:00 | first remote matrix | GitHub Actions run `29545099209` | fail | Windows FPC installed below `C:\tools\freepascal`; initial discovery path was incorrect |
| 2026-07-17T07:41:00+07:00 | Windows FPC package audit | GitHub Actions run `29545430349` | diagnosed | `fpjson.ppu` existed, but Chocolatey generated no `fpc.cfg`; harness now accepts explicit unit roots |
| 2026-07-17T07:52:00+07:00 | Windows UTF-8 framing audit | GitHub Actions runs `29545642377`–`29545994537` | diagnosed | string concatenation transcoded raw payload bytes and the decoded string inherited the system code page; byte-copy framing and explicit UTF-8 tagging fixed both |
| 2026-07-17T07:57:57+07:00 | cross-platform release gate | GitHub Actions run `29546115367` | pass | TypeScript, Ubuntu, macOS, and Windows jobs all completed successfully |
...
| `.phase0/**` | isolated downloaded/generated runtime and evidence | completed locally; ignored |
| `extension/**` | TypeScript client | completed |
| `server/**` | Pascal language server | completed |
| `tests/**` | protocol/text/parser fixtures | completed |
| `tools/**` | repeatable launch/validation scripts | completed |
| `.github/workflows/**` | cross-platform CI definition | completed; remote matrix green |
...
~90% — calibrated guess, not a guarantee.
...
- Unknowns pushing confidence down: the no-impact isolation constraint was breached twice during discovery, the parser remains a Phase 0 proof rather than a complete grammar, and Kotlin/IntelliJ parity is intentionally unmeasured until equivalent features exist.
- What would change this estimate: Phase 1 incremental syntax-tree equivalence and latency gates raise it; new isolation or cross-platform regressions lower it.
...
- Existing VS Code state was affected by the accidental raw GUI launch and the later discovery of the separate home-level shared store. The impact is limited to observed cache/log/state writes, but precise semantic state changes cannot be reconstructed without a before snapshot.
- GitHub reports non-blocking deprecation warnings for the current major versions of the official checkout/setup actions; migrate them separately when their next stable majors are selected.

## 12. Resume Point

- Last completed step: passed local editor/runtime validation and the full Ubuntu/macOS/Windows remote matrix, then updated the durable report
- Next safe step: use the approved roadmap to scope and begin Phase 1 syntax support
- Known blockers: none for Phase 0 completion
- Commands to rerun: `./tools/validate.sh --with-editor`
- Unverified assumptions: Kotlin/IntelliJ comparative performance until equivalent FPC XUI feature slices exist
...
- 2026-07-17T07:57:57+07:00 — recorded Windows FPC discovery, raw UTF-8 framing repairs, and the fully green cross-platform CI gate
